...
> - **トークンの管理は自己責任です。** 認証トークンが漏洩し、かつサーバーにネットワーク的に到達可能な第三者がいる場合、あなたの PC 上で Claude Code を操作されるおそれがあります。信頼できる LAN 内のみで運用していればリスクは限定的ですが、トークンが漏洩した疑いがある場合は、直ちにサーバーを停止し、`.env` の `AUTH_TOKEN` を変更するか、削除して再起動してください（未設定なら起動時に新しいトークンが自動生成されます）。
>
...
- PC から離れて家事や子育てなど別の作業をしながら、スマートフォンで Claude Code に指示を出し続けることができます
...
| `CLAUDE_PATH` | `claude` コマンドのパス。シェルの PATH で解決できない場合に指定 | `claude` |
